...
for line in raw.split("\n"):
    ...
    if len(parts) >= 2:
        ...
        prefix_yt = "#HttpOnly_.youtube.com" if name.startswith("__Secure") else ".youtube.com"
        prefix_go = "#HttpOnly_.google.com"  if name.startswith("__Secure") else ".google.com"

        out.append(f"{prefix_yt}\tTRUE\t/\tTRUE\t{expire}\t{name}\t{val}")
        
        # Mirror core auth cookies to Google domain
        if name in ["__Secure-1PSID", "__Secure-3PSID", "SID", "HSID", "SSID", "SAPISID", "APISID"]:
            out.append(f"{prefix_go}\tTRUE\t/\tTRUE\t{expire}\t{name}\t{val}")

with open("output_mirrored.txt", "w") as f:
    f.write("\n".join(out) + "\n")
